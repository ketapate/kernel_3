...
special_file_read(vnode_t *file, off_t offset, void *buf, size_t count)
{
    ...
    KASSERT(file);
    dbg(DBG_PRINT,"(GRADING2A 1.a) The file is not NULL. \n");
    
    /* grading guideline required */
    KASSERT((S_ISCHR(file->vn_mode) || S_ISBLK(file->vn_mode)));
    dbg(DBG_PRINT,"(GRADING2A 1.a) The file's type is either character special or block special. \n");
    
    /* grading guideline required */
    if (S_ISCHR(file->vn_mode)) {
        KASSERT(file->vn_cdev && file->vn_cdev->cd_ops && file->vn_cdev->cd_ops->read);
        dbg(DBG_PRINT,"(GRADING2A 1.a) This file is character special, and do has read function. \n");
    }
...
special_file_write(vnode_t *file, off_t offset, const void *buf, size_t count)
{
    ...
    /* grading guideline required */
    KASSERT(file);
    dbg(DBG_PRINT,"(GRADING2A 1.b) The file is not NULL. \n");
    
    /* grading guideline required */
    KASSERT((S_ISCHR(file->vn_mode) || S_ISBLK(file->vn_mode)));
    dbg(DBG_PRINT,"(GRADING2A 1.b) The file's type is either character special or block special. \n");
    
    /* grading guideline required */
    if (S_ISCHR(file->vn_mode)) {
        KASSERT(file->vn_cdev && file->vn_cdev->cd_ops && file->vn_cdev->cd_ops->write);
        dbg(DBG_PRINT,"(GRADING2A 1.b) This file is character special, and do has write function. \n");
    }
...
special_file_mmap(vnode_t *file, vmarea_t *vma, mmobj_t **ret)
{
    ...
    KASSERT(file);
    dbg(DBG_PRINT, "(GRADING3A 5.a) file exists \n");
    KASSERT(S_ISCHR(file->vn_mode) && "because these ops only assigned if vnode represents a special file");
    dbg(DBG_PRINT, "(GRADING3A 5.a) file->vn_mode is character special file \n");
    KASSERT((file->vn_cdev) && "because open shouldn\'t have let us arrive here if vn_cdev was NULL");
    dbg(DBG_PRINT, "(GRADING3A 5.a) file->vn_cdev exists \n");
    KASSERT(file->vn_cdev->cd_ops && file->vn_cdev->cd_ops->mmap);
    dbg(DBG_PRINT, "(GRADING3A 5.a) file->vn_cdev->cd_ops exists and file->vn_cdev->cd_ops->mmap exists \n");
...
special_file_fillpage(vnode_t *file, off_t offset, void *pagebuf)
{
    ...
    KASSERT(file);
    dbg(DBG_PRINT, "(GRADING3A 5.b) file exists \n");
    KASSERT(S_ISCHR(file->vn_mode));
    dbg(DBG_PRINT, "(GRADING3A 5.b) file->vn_mode is character special file \n");
    KASSERT((file->vn_cdev));
    dbg(DBG_PRINT, "(GRADING3A 5.b) file->vn_cdev exists \n");
    KASSERT(file->vn_cdev->cd_ops && file->vn_cdev->cd_ops->fillpage);
    dbg(DBG_PRINT, "(GRADING3A 5.b) file->vn_cdev->cd_ops exists and file->vn_cdev->cd_ops->fillpage exists \n");
...
special_file_dirtypage(vnode_t *file, off_t offset)
{
    ...
    KASSERT(file);
    dbg(DBG_PRINT, "(GRADING3A 5.c) file exists \n");
    KASSERT(S_ISCHR(file->vn_mode));
    dbg(DBG_PRINT, "(GRADING3A 5.c) file->vn_mode is character special file \n");
    KASSERT((file->vn_cdev));
    dbg(DBG_PRINT, "(GRADING3A 5.c) file->vn_cdev exists \n");
    KASSERT(file->vn_cdev->cd_ops && file->vn_cdev->cd_ops->dirtypage);
    dbg(DBG_PRINT, "(GRADING3A 5.c) file->vn_cdev->cd_ops exists and file->vn_cdev->cd_ops->dirtypage exists \n");
...
special_file_cleanpage(vnode_t *file, off_t offset, void *pagebuf)
{
    ...
    KASSERT(file);
    dbg(DBG_PRINT, "(GRADING3A 5.d) file exists \n");
    KASSERT(S_ISCHR(file->vn_mode));
    dbg(DBG_PRINT, "(GRADING3A 5.d) file->vn_mode is character special file \n");
    KASSERT((file->vn_cdev));
    dbg(DBG_PRINT, "(GRADING3A 5.d) file->vn_cdev exists \n");
    KASSERT(file->vn_cdev->cd_ops && file->vn_cdev->cd_ops->cleanpage);
    dbg(DBG_PRINT, "(GRADING3A 5.d) file->vn_cdev->cd_ops exists and file->vn_cdev->cd_ops->cleanpage exists \n");
