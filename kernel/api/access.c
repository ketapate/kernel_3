...
int addr_perm(struct proc *p, const void *vaddr, int perm)
{
    ...
    /*You need to find the process's
     * vm_area that contains that virtual address, and verify that the protections
     * allow access.*/
    ...
    vma = vmmap_lookup(p->p_vmmap, vfn); 
    /* Corrected */
    if (vma == NULL) {
        dbg(DBG_PRINT, "addr_perm failed because vmmap_lookup failed \n");
        return 0;
    }
    
    
    /*The page protections need not match the specified permissions
     * exactly, as long as at least the specifed permissions are satisfied.*/
    
    if ((vma->vma_prot & perm) != perm){
        dbg(DBG_PRINT, "vma->vm_prot = %x does not satisfy perm = %x\n", vma->vma_prot, perm);
        return 0;
    } else {
        dbg(DBG_PRINT, "vma->vm_prot = %x satisfies perm = %x\n", vma->vma_prot, perm);
        return 1;
    }
    dbg(DBG_PRINT, "Should not reach here\n");
    return 0;
}
...
int range_perm(struct proc *p, const void *avaddr, size_t len, int perm)
{
    ...
    /* The following implementation will be concise and with better time complexity */
    uint32_t startvfn = ADDR_TO_PN(avaddr);
    uint32_t endvfn = ADDR_TO_PN((uint32_t)avaddr + len - 1);
    uint32_t i;
    
    for (i = startvfn; i <= endvfn; i++) {
        if (addr_perm(p, PN_TO_ADDR(i), perm) == 0) {
            return 0;
        }
    }
    return 1;
